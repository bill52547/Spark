...
__device__
float cal_dist2(const float x1, const float y1, const float z1,
...
__device__ void
project_device(float *vproj, const float *image, const float x1l,
               ...
               const float z2b, const float z2f, const int nx, const int ny,
               const int nz, const float dx, const float dy, const float dz,
    ...
    const float over_sqrt_2pi_tof_sigma2 =
            1 / sqrtf(2.0 * PI * tof_sigma * tof_sigma);
    const float L = cal_dist2(x1c, y1c, z1c, x2c, y2c, z2c);
    ...
    if (ABS(x1c - x2c) > ABS(y1c - y2c)) {
        ...
        for (int ix = 0; ix < nx; ix++) {
            ...
            if (tof_sigma > 0) {
                ...
                // tof_weight =(1 + t2 + t2 * t2 / 2 + t2 * t2 * t2 / 6) *
                // over_sqrt_2pi_tof_sigma2;
                tof_weight =
            ...
            for (int iy = MAX(0, Yi1); iy <= MIN(ny - 1, Yi2); iy++) {
                ...
                for (int iz = MAX(0, Zi1); iz <= MIN(nz - 1, Zi2); iz++) {
                    ...
                    wz /= (zi2 - zi1);
                    vproj[0] += image[ix + iy * nx + iz * nx * ny] * wy * wz * weight;
                    // sum_weight += wy * wz;
                }
            }
        }
        ...
        for (int iy = 0; iy < ny; iy++) {
            ...
            if (tof_sigma > 0) {
                ...
                float t2 = -d2_tof * d2_tof * over_tof_sigma2;
                // tof_weight = (1 - 0.5 * t2 + 0.25 * t2 * t2 / 2 - 0.125 * t2 * t2 *
                // t2 / 6) * over_sqrt_2pi_tof_sigma2;
                tof_weight =
            ...
            for (int ix = MAX(0, Xi1); ix <= MIN(nx - 1, Xi2); ix++) {
                ...
                for (int iz = MAX(0, Zi1); iz <= MIN(nz - 1, Zi2); iz++) {
                    wz = MIN((iz + 1.0f) * dz, zi2) - MAX((iz + 0.0f) * dz, zi1);
                    wz /= (zi2 - zi1);
                    vproj[0] += image[ix + iy * nx + iz * nx * ny] * wx * wz * weight;
                }
            }
        }
    }
    // if (sum_weight > 0)
    // {vproj[0] *= sum_weight * sum_weight;}
}
__global__ void project(DListmode listmode,
                        const DImage image,
                        const float ctr = -1.f)
{
    ...
    for (int tid = blockIdx.x * blockDim.x + threadIdx.x;
        ...
        IndexType id1 = listmode.index1[tid], id2 = listmode.index2[tid];
        float time_diff_ = 0.f;
        if (ctr > 0) {time_diff_ = listmode.time_diff[tid];}
        if (tid >= nx * ny * nz) {return;}
        project_device(listmode.data + tid,
                       image.data,
...
Listmode Projector::operator() (Image & image)
{
    device_vector<float> out_data(this->lor.GetNum());
    DImage d_image = {image.GetShape().x,
                      image.GetShape().y,
                      image.GetShape().z,
                      thrust::raw_pointer_cast(image.GetAffine().data.data()),
                      image.GetDataRawPtr()};

    DListmode d_listmode = {
            this->lor.GetNum(),
            thrust::raw_pointer_cast(out_data.data()),
            this->lor.GetIndex1RawPtr(),
            this->lor.GetIndex2RawPtr(),
            this->lor.GetTimeDiffRawPtr(),
            this->lor.GetLutPosRawPtr()
    };

    project<<<this->lor.GetNum() / 512 + 1 ,512>>>(d_listmode, d_image, this->CTR);
    CHECK_LAUNCH_ERROR();
    return Listmode(this->lor, out_data);
}
